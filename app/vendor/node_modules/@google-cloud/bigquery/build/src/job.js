...
class Job extends common_1.Operation {
    constructor(bigQuery, id, options) {
        ...
        this.getQueryResultsStream =
            paginator_1.paginator.streamify('getQueryResultsAsStream_');
    }
    /**
    ...
    getQueryResults(optionsOrCallback, cb) {
        ...
        this.bigQuery.request({
            ...
            if (resp.schema && resp.rows) {
                rows = src_1.BigQuery.mergeSchemaWithRows_(resp.schema, resp.rows);
            }
            let nextQuery = null;
    ...
    poll_(callback) {
        this.getMetadata((err, metadata, apiResponse) => {
            ...
            if (!err && apiResponse.status &&
                ...
                // tslint:disable-next-line no-any
                err = new common_1.util.ApiError(apiResponse.status);
            }
            if (err) {
                callback(err);
                return;
